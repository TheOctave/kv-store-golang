...
func loadData(ctx context.Context) (map[string]string, error) {
	...
	// First check if the folder exists and create if it's missing
	...
	// Then check if the file exists and create it if it's missing.
...
func saveData(ctx context.Context, data map[string]string) error {
	// First check if the folder exists and create it if its missing
	if _, err := os.Stat(storagePath); os.IsNotExist(err) {
		err = os.MkdirAll(storagePath, 0755)
		if err != nil {
			return err
